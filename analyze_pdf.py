...
def analyze(prompt, text_content, api_key=None):
...
def main():
  ...
  try:
    ...
    if output_path.exists():
      print(f"Found existing analysis at '{output_file}', using it...")
      with open(output_path, "r", encoding="utf-8") as f:
        response = f.read()
    else:
      print("No existing analysis found, calling Gemini API...")
      response = analyze(prompt, pdf_text)

    # embeddings = generate_embeddings(response)
    save(pdf_name, response)
